...
#include "include/charger/max77843_charger.h"
...
static void max77843_set_charger_state(struct max77843_charger_data *charger,
				       int enable);
static void max77843_enable_aicl_irq(struct max77843_charger_data *charger);
...
static bool max77843_is_constant_current(struct max77843_charger_data *charger)
{
	u8 reg_data;
...
static int max77843_get_charging_health(struct max77843_charger_data *charger)
{
	...
	/* watchdog kick */
	max77843_update_reg(charger->i2c, MAX77843_CHG_REG_CNFG_06,
			MAX77843_WDTCLR, MAX77843_WDTCLR);
	...
	if (state == POWER_SUPPLY_HEALTH_GOOD) {
		...
		if((charger->is_charging == 1) && ((chg_dtls & 0x08) || (chg_dtls & 0x04))) {
			...
			max77843_set_charger_state(charger, DISABLE);
			max77843_set_float_voltage(charger, charger->float_voltage);
			max77843_set_charger_state(charger, ENABLE);
		}
...
static void reduce_input_current(struct max77843_charger_data *charger, int cur)
{
	...
	if (!max77843_read_reg(charger->i2c, set_reg, &set_value)) {
		...
		charger->input_current = max77843_get_input_current(charger);
		...
		int path)
{
	...
	max77843_read_reg(charger->i2c, MAX77843_CHG_REG_CNFG_12, &cnfg12);
				       ...
				       int input_current)
{
	...
	if (!input_current) {
		...
		input_current = (input_current > 1260) ? 1260 : input_current;
		quotient = input_current / 20;
		reg_data |= quotient;
		max77843_write_reg(charger->i2c, set_reg, reg_data);
	} else {
...
static void max77843_set_wireless_input_current(struct max77843_charger_data *charger,
				       int input_current)
{
	union power_supply_propval value;
	...
	if (is_wireless_type(charger->cable_type)) {
		...
		if (charger->wc_current > charger->wc_pre_current) {
			max77843_set_charge_current(charger, charger->charging_current);
		}
	}
	...
	int enable)
{
	...
	max77843_read_reg(charger->i2c, MAX77843_CHG_REG_CNFG_00, &cnfg_00);
	pr_info("%s : CHG_CNFG_00(0x%02x)\n", __func__, cnfg_00);
}
...
static void max77843_charger_initialize(struct max77843_charger_data *charger)
{
	 ...
	 */
	max77843_set_float_voltage(charger, charger->pdata->chg_float_voltage);
	...
	case DATA:
		if (sscanf(buf, "0x%x 0x%x", &x, &y) == 2) {
			if (x >= 0xB1 && x <= 0xC3) {
				u8 addr = x;
				u8 data = y;
				if (max77843_write_reg(charger->i2c, addr, data) < 0) {
			      ...
			      union power_supply_propval *val)
{
...
#if defined(CONFIG_MAX77843_FG_SENSING_WA)
		max77843_current_sensing_wa(charger);
#endif
		max77843_read_reg(charger->i2c,
				  MAX77843_CHG_REG_DETAILS_01, &reg_data);
		reg_data &= 0x0F;
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE:
		if (max77843_is_constant_current(charger)) {
			val->intval = 0;
		} else {
			val->intval = 1;
		}
			  ...
			  const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		...
		max77843_set_buck(charger, buck_state);
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
	{
		...
		if (is_wireless_type(charger->cable_type)) {
			max77843_set_wireless_input_current(charger, input_current);
		} else {
			max77843_set_input_current(charger, input_current);
		}
		if (charger->cable_type == POWER_SUPPLY_TYPE_BATTERY)
			max77843_set_wireless_input_current(charger, input_current);
	...
	case POWER_SUPPLY_PROP_CURRENT_FULL:
		max77843_set_topoff_current(charger, val->intval);
		break;
#if defined(CONFIG_AFC_CHARGER_MODE)
	case POWER_SUPPLY_PROP_AFC_CHARGER_MODE:
...
static void wpc_detect_work(struct work_struct *work)
{
	...
	if (is_wireless_type(charger->cable_type)) {
		...
		do {
			wcin_cnt++;
			max77843_read_reg(charger->i2c, MAX77843_CHG_REG_INT_OK,
				&reg_data);
			wcin_state = (reg_data & MAX77843_WCIN_OK) >> MAX77843_WCIN_OK_SHIFT;
			max77843_read_reg(charger->i2c, MAX77843_CHG_REG_DETAILS_00,
				&reg_data);
			wcin_dtls = (reg_data & MAX77843_WCIN_DTLS) >> MAX77843_WCIN_DTLS_SHIFT;
...
static void max77843_aicl_isr_work(struct work_struct *work)
{
	...
	while (!(aicl_state & 0x80) && charger->cable_type != POWER_SUPPLY_TYPE_BATTERY) {
		...
		mdelay(50);
		max77843_read_reg(charger->i2c, MAX77843_CHG_REG_INT_OK, &aicl_state);
		if (max77843_get_input_current(charger) <= MINIMUM_INPUT_CURRENT)
			break;
	}

	if (charger->aicl_on) {
		union power_supply_propval value;
		value.intval = max77843_get_input_current(charger);
		psy_do_property("battery", set,
			POWER_SUPPLY_EXT_PROP_AICL_CURRENT, value);
	}

	if (is_not_wireless_type(charger->cable_type))
		max77843_check_slow_charging(charger, charger->input_current);
...
static int __devinit max77843_charger_probe(struct platform_device *pdev)
{
	...
	max77843_read_reg(charger->i2c, MAX77843_CHG_REG_INT_OK, &reg_data);
	if (reg_data & MAX77843_WCIN_OK)
		charger->cable_type = POWER_SUPPLY_TYPE_WIRELESS;
	charger->input_current = max77843_get_input_current(charger);
	charger->charging_current = max77843_get_charge_current(charger);
	...
	wake_lock_init(&charger->aicl_wake_lock, WAKE_LOCK_SUSPEND,
		"charger-aicl");
	INIT_DELAYED_WORK(&charger->aicl_work, max77843_aicl_isr_work);
